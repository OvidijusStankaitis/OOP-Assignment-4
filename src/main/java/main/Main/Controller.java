...
public class Controller {
    ...
    private void addAttendanceColumn(LocalDate date) {
        ...
        for (TableColumn<Student, ?> column : studentTable.getColumns()) {
            if (column.getText().equals(dateString)) {
                return; // Date column already exists, exit the method
            }
        }
        ...
        // Set the cell factory to use TextFieldTableCell for editing
        attendanceColumn.setCellFactory(TextFieldTableCell.forTableColumn());

        // Handle the edit commit event
        attendanceColumn.setOnEditCommit(event -> {
            Student student = event.getRowValue();
            String newValue = event.getNewValue();

            // Update the attendance value for the student on the given date
            student.addAttendance(dateString, newValue);
        });
    ...
    private void showStudents(StudentGroup group) {
        ...
        // Collect the date columns from the attendance data
        List<String> dateColumns = new ArrayList<>();
        Set<String> allDates = new HashSet<>();
        for (Student student : group.getStudents()) {
            allDates.addAll(student.getAttendance().keySet());
        }
        dateColumns.addAll(allDates);

        // Sort the date columns in ascending order
        Collections.sort(dateColumns);

        // Add the date columns to the table in ascending order
    ...
    public void exportStudentDataToExcel(ObservableList<Student> students, String fileName) {
        ...
        Set<String> attendanceDates = new HashSet<>();

        // Add all unique dates from students' attendance
        for (Student student : students) {
            attendanceDates.addAll(student.getAttendance().keySet());
        }

        ArrayList<String> headers = new ArrayList<>(attendanceDates);
        Collections.sort(headers);

        // Add basic fields at the start
        headers.add(0, "Student ID");
        headers.add(0, "Surname");
        headers.add(0, "Name");
        ...
        for (int i = 0; i < students.size(); i++) {
            ...
            Map<String, StringProperty> attendance = student.getAttendance();
            for (int j = 3; j < headers.size(); j++) {
                String date = headers.get(j);
                StringProperty attendanceProperty = attendance.get(date);
                String attendanceValue = attendanceProperty != null ? attendanceProperty.get() : "";
                row.createCell(j).setCellValue(attendanceValue);
            }
        }
        ...
        try (InputStream inputStream = new FileInputStream(fileName)) {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                if (row != null) {
                    ...
                    for (int j = 3; j < headers.size(); j++) {
                        String date = headers.get(j);
                        String attendance = row.getCell(j).getStringCellValue();
                        student.getAttendance().put(date, new SimpleStringProperty(attendance));
                    }
